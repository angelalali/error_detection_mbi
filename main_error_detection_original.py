...
for col in ed.used_cols:
    ...
    if col.strip() in ['Material', 'Description', 'Follow-up matl', 'Maintenance status']:
        continue
...
df_global_histogram_errors.to_csv(config.global_histogram_errors_file, index=False, columns=all_columns, encoding='utf-8')
...
for col in ed.df.columns:
    ...
    # skip some columns
    if col.strip() in ['Material', 'Plant', 'Description', 'Follow-up matl', 'Material Type', 'MRP Controller', 'Lot size', 'MRP group']:
        continue
