...
special_col = ['Material', 'Plant Description', 'Description', 'Follow-up matl', 'Valid from',
               'Maintenance status', 'Eff.-out', 'MRP Controller', 'Lot size', 'MRP group']
...
def relabeledDataFrame(df, colTypes):
    ...
    for col in df.columns:
        ...
        if colTypes[col] == 'BOOLEAN':
            df_relabeled[col] = df[col].apply(lambda x: int(x == 'X' or x == 1 or x=='T'))
...
#     # print 'total fields used: ', len(cols)
#     # print cols
#
